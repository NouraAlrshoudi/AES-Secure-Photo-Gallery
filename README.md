...
def apply_style(root, theme):
    ...
    if theme == "dark":
        ...
        style.configure("TButton", background="#3a3a3a", foreground="#ffffff",
                        padding=8, font=("Segoe UI", 11))
        style.map("TButton", background=[("active", "#5a5a5a")])
        style.configure("TEntry", fieldbackground="#2c2c2c", foreground="#ffffff",
                        font=("Segoe UI", 11))
...
def main_app():
    ...
    root = tk.Tk()
    root.title("Secure Photo Encryptor/Decryptor")
    ...
    def upload_file():
        path = filedialog.askopenfilename(
            title="Select Photo or Encrypted Photo",
            filetypes=[
                ("Images", "*.png *.jpg *.jpeg *.gif *.bmp *.webp"),
                ("Encrypted Photos", "*.enc"),
                ("All Files", "*.*")
            ]
        )
        if path:
            if not is_image_path(path):
                messagebox.showwarning("Invalid", "Please select an image file only.")
                return
    ...
    def encrypt_file_action():
        ...
        if not path:
            messagebox.showwarning("No File", "Please upload a photo first.")
            return
        if path.lower().endswith(".enc"):
            messagebox.showwarning("Warning", "This file is already encrypted (.enc).")
            return

        _, ext = os.path.splitext(path)
        if ext.lower() not in ALLOWED_EXT:
            messagebox.showwarning("Invalid", "Only image files are allowed.")
            return

        out = encrypt_file(path)
        messagebox.showinfo("Encrypted", f"Encrypted photo saved as:\n{out}")

    def decrypt_file_action():
        path = uploaded_file.get()
        if not path:
            messagebox.showwarning("No File", "Please upload an encrypted photo first.")
            return
        if not path.lower().endswith(".enc"):
            messagebox.showwarning("Invalid", "Please select an encrypted photo (.enc).")
            return

        out = decrypt_file(path)
        if out:
            messagebox.showinfo("Decrypted", f"Decrypted photo saved as:\n{out}")

    ttk.Label(frame, text="Secure Your Photos!", font=("Segoe UI", 20, "bold")).pack(pady=10)
    ttk.Button(frame, text="Upload Photo / Encrypted Photo", command=upload_file, width=30).pack(pady=5)
    ttk.Button(frame, text="Encrypt Photo", command=encrypt_file_action, width=30).pack(pady=5)
    ttk.Button(frame, text="Decrypt Photo", command=decrypt_file_action, width=30).pack(pady=5)
